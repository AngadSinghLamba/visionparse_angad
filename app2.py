# app.py  ── VisionParse Final ---------------------------------------------
# Bulk upload → MD, HTML, TXT, JSON, optional XML + assets (tables/images) → ZIP
# Multimodal Parquet export optional
# Images in Markdown/HTML are always REFERENCED (external files)
# ----------------------------------------------------------------------------
...
class VisionParseConfig:
    SUPPORTED_EXTS: List[str] = field(default_factory=lambda: [
        "pdf", "docx", "xlsx", "html", "htm", "pptx", "md", "csv", "png", "jpg", "jpeg", "asciidoc"
    ])
...
@st.cache_resource(show_spinner="🔧 Loading Docling & OCR pipeline…")
...
def generate_txt(res, base: Path):
    ...
    for el, _ in res.document.iterate_items():
        ...
        if isinstance(lvl, int) and txt:
            ...
            lines.append(f"[Table {it}]")
            lines.append(df.to_string(index=False))
        elif isinstance(el, PictureItem):
            fn = imgs[pt].name if pt < len(imgs) else f"{base.name}_img_{pt+1}.png"
            pt += 1
...
def main():
    ...
    if st.button("🚀 Convert All", disabled=not st.session_state.files):
        ...
        with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_DEFLATED) as zf:
            for doc_dir in out_root.iterdir():
                ...
                for file_path in doc_dir.iterdir():
                    ...
                    if ext == ".md":     arc = f"{prefix}/md/{file_path.name}"
                    elif ext == ".html": arc = f"{prefix}/html/{file_path.name}"
                    elif ext == ".json": arc = f"{prefix}/json/{file_path.name}"
                    elif ext == ".txt":  arc = f"{prefix}/txt/{file_path.name}"
                    elif ext == ".csv":  arc = f"{prefix}/assets/tables/{file_path.name}"
        ...
        st.success("✅ All done!")
        st.markdown(make_zip_link(zip_buf.getvalue()), unsafe_allow_html=True)
